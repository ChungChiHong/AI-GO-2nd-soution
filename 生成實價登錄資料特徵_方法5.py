        ...
        for z in ["+","-"]:
            ...
            for index,y in enumerate(Y):
                ...
                if z=="+":
                    ...
                    if index==0:
                        part_of_external_df1 = part_of_external_df[(part_of_external_df["屋齡"]-row["屋齡"]<=y)&\
                                                  (part_of_external_df["屋齡"]-row["屋齡"]>=0)]
                   
                    else:
                        part_of_external_df1 = part_of_external_df[(part_of_external_df["屋齡"]-row["屋齡"]>=Y[index-1])&\
                    ...
                    if len(part_of_external_df1)>0:
                        ...
                        I=[0.25,0.5,0.75,1,2,3]
                        for index1,i in enumerate(I):
                            if index1==0:
                                df_ = part_of_external_df1.iloc[np.where(distances<=i)[0]]
                            
                            else:
                                df_ = part_of_external_df1.iloc[np.where((distances<=i)&(distances>=I[index1-1]))[0]]
                            ...
                            if count!=0:
                                ...
                                if index==0:
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，平均單價"] = avg_price
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，最大單價"] = max_price
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，最小單價"] = min_price
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，單價中位數"] = median_price
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，交易次數"] = count
                                    df.loc[idx, f"{s}，屋齡大於0~{y}年，單價標準差"] = std_price
                               
                                else:
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，平均單價"] = avg_price
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，最大單價"] = max_price
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，最小單價"] = min_price
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，單價中位數"] = median_price
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，交易次數"] = count
                                    df.loc[idx, f"{s}，屋齡大於{Y[index-1]}~{y}年，單價標準差"] = std_price
                        
                else:
                  
                    if index==0:
                        part_of_external_df1 = part_of_external_df[(row["屋齡"]-part_of_external_df["屋齡"]<=y)&\
                                                  (row["屋齡"]-part_of_external_df["屋齡"]>=0)]
                  
                    else:
                        part_of_external_df1 = part_of_external_df[(row["屋齡"]-part_of_external_df["屋齡"]>=Y[index-1])&\
                    ...
                    if len(part_of_external_df1)>0:
                        ...
                        for i in [0.25,0.5,0.75,1,2,3]:
                            ...
                            if count!=0:
                                ...
                                if index==0:
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，平均單價"] = avg_price
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，最大單價"] = max_price
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，最小單價"] = min_price
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，單價中位數"] = median_price
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，交易次數"] = count
                                    df.loc[idx, f"{s}，屋齡小於0~{y}年，單價標準差"] = std_price

                                else:
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，平均單價"] = avg_price
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，最大單價"] = max_price
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，最小單價"] = min_price
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，單價中位數"] = median_price
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，交易次數"] = count
                                    df.loc[idx, f"{s}，屋齡小於{Y[index-1]}~{y}年，單價標準差"] = std_price
